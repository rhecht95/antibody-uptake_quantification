...
all_temp_exps <- lapply(all_temp_exps, function(x){
  x %>%   
    ...
    mutate(tissue_avg = mean(sum_intensities_3/volume_volume_um3)) %>% 
    ungroup() %>% 
    group_by(image_set) %>% 
    mutate(img_avg = mean(sum_intensities_3/volume_volume_um3)) %>% 
    ungroup()
  })
...
all_temp_exps <- all_temp_exps %>% 
    ...
    mutate(condition_avg = mean(tissue_avg),
           ...
           condition_sem = std.error(tissue_avg)) %>%
    ungroup()

# std.error is being calculated incorrectly above. The real result should be this
all_temp_exps %>% 
  filter(condition == "4 ºC") %>% 
  pull(tissue_avg) %>% 
  unique() %>% 
  std.error()
```
...
temp_plot <- uptake_plot(all_temp_exps, output_name = "all_temp_exps_V4.1_sums_se", condition_label = "Incubation Temperature", add_stats = TRUE, temp_t_test)
temp_plot 
```
Table of the plotted values
```{r}
all_temp_exps %>% 
  group_by(tissue, condition, condition_avg, tissue_avg, condition_sd,condition_sem, exp) %>%
  summarise() %>% 
  kbl() %>% 
  kable_paper()
```
