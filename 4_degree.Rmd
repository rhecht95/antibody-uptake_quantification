...
This document has been updated to analyze data procesed by Arivis pipeline:  "Ab-uptake_split-retina_V5.1.pipeline"
...
Take `sum_intensities_3`, which represents the total amount of 488/uptake signal in an image (after filtering), and divide by the total number of ON-BCs in the image
...
Next, group by `tissue` and average `sum_intensities_3`. Also group by `image_set` and average `mean_intensities_3`
```{r}
...
temp_plot <- uptake_plot(all_temp_exps, output_name = "all_temp_exps_V5", condition_label = "Incubation Temperature", add_stats = TRUE, temp_t_test)
temp_plot 
```
