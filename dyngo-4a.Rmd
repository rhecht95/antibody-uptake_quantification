...
#e1-456
DMSO_imgs <- c("s2.2", "s2.3") #removed s2.1
dyngo_imgs <- c("s3.3")
...
all_dyngo_exps %>% 
  ...
  geom_col(aes(x = image_set, y = avg_upc), fill = "blue")+
...
all_dyngo_exps %>% 
  ...
  summarise() %>% 
  arrange(exp) %>% 
  filter((exp == "e449" & tissue == "s2.1") | (exp == "e456" & tissue == "s2.3")) %>% 
  ggplot()+
  #geom_col(aes(x = image_set, y = tissue_avg), fill = "orange")+
  geom_col(aes(x = image_set, y = sum_intensities), fill = "blue")+
  #geom_col(aes(x = image_set, y = on_bc_count/on_bc_count[1]), fill = "darkgreen")
  theme(axis.text.x = element_text(angle = 90))

#ggsave("/Users/ryanhecht/Desktop/sum-intensities.pdf")
```
...
```
Visually, the data appears very close to normally distributed. The Shapiro-Wilk test does not agree but it is very close to the threshold and apparently these tests lack sensitivity for small sample sizes. 
