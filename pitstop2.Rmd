...
In this project, the effect of inhibiting clathrin mediated endocytosis is observed by treating split retinas with Pitstop 2. Briefly, the photoreceptors were removed from freshly isolated mouse retinas which were then incubated with an antibody for 1hr in the presence or absence of Pitstop 2. The retinas were washed, then incubated with a viability dye to track cell death. Afterward, retinas were fixed, and the localization of the internalized antibodies was revealed by incubating the tissue with a labeled secondary antibody that targets the internalized antibody. 
...
The `read_n_clean` function also implements the `condition_select` method, which allows the user to assign treatment conditions (i.e DMSO vs pitstop2) to samples by the name of the sample (i.e "s1.2")
```{r}
#e1-469
DMSO_imgs <- c("s1.1","s1.3","s2.1","s2.3")
pitstop2_imgs <- c("s3.1","s3.3","s4.1","s4.3")
e469 <- read_n_clean(folder_path = "data/e1-469", cond1_vec = DMSO_imgs, cond2_vec = pitstop2_imgs, cond1_title = "DMSO", cond2_title = "pitstop2")

#e1-475
DMSO_imgs <- c("s1.3")
pitstop2_imgs <- c("s3.3")
e475 <- read_n_clean(folder_path = "data/e1-475", cond1_vec = DMSO_imgs, cond2_vec = pitstop2_imgs, cond1_title = "DMSO", cond2_title = "pitstop2")

#e1-478
DMSO_imgs <- c("s2.1", "s2.2", "s2.3")
pitstop2_imgs <- c("s3.1", "s3.3", "s4.2")
e478 <- read_n_clean(folder_path = "data/e1-478", cond1_vec = DMSO_imgs, cond2_vec = pitstop2_imgs, cond1_title = "DMSO", cond2_title = "pitstop2")

#place all experiment dataframes into a list for batch processing
all_pitstop2_exps <- list("e469" = e469, "e475" = e475, "e478" = e478)
```
...
all_pitstop2_exps <- lapply(all_pitstop2_exps, function(x){mutate(x, tissue = str_extract(x$image_set, pattern = sample_string),.after = 1)})
all_pitstop2_exps[1]
```
...
```{r}
all_pitstop2_exps <- lapply(all_pitstop2_exps, avg_uptake)
```

Merge all dataframes in list into one dataframe
```{r}
all_pitstop2_exps <- bind_rows(all_pitstop2_exps, .id = "exp") %>% 
...
all_pitstop2_exps["avg_on_bc_count"] <- mean(all_pitstop2_exps$on_bc_count) 

#average uptake per cell, sum intensity for a single image over the avg ON-BC count for all experiments
all_pitstop2_exps["avg_upc"] <- all_pitstop2_exps$sum_intensities/all_pitstop2_exps$avg_on_bc_count 
```
...
```{r}
all_pitstop2_exps <- all_pitstop2_exps %>% 
...
```{r}
shapiro_result <- all_pitstop2_exps %>% 
  group_by(tissue, condition, condition_avg, tissue_avg, condition_sd, exp) %>%
  summarise() %>% 
  pull(tissue_avg) %>% 
  shapiro.test()
...
```{r}
pitstop2_t_test <- all_pitstop2_exps %>% 
  group_by(tissue, condition, condition_avg, tissue_avg, condition_sd, exp) %>%
...
pitstop2_plot <- uptake_plot(all_pitstop2_exps, output_name = "all_pitstop2_exps_ML_V2",condition_label = "", add_stats = TRUE, pitstop2_t_test)
pitstop2_plot 
```
